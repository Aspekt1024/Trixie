...
namespace Aspekt.AI.Editor
{
    public class AIGUI
    {
        public const float GRID_SPACING = 10f;
        public const float RowHeight = 20f; // Used for interfaces and node layout
        ...
        private static AIGUI _instance;
        private static bool _appQuitting = false;
        private static object _lock = new object();

        public static AIGUI Instance
        {
            get
            {
        ...
        public static float VerticalSlider(Rect rect, float value, float minValue, float maxValue, float step, string label)
        {
            ...
            GUI.Label(new Rect(pos + new Vector2(15f, 0f), new Vector2(50f, 20f)), label);
            value = EditorGUI.FloatField(new Rect(pos + new Vector2(15f, 20f), new Vector2(35f, 15f)), value);

            value = GUI.VerticalSlider(new Rect(pos, rect.size), value, maxValue, minValue);
            value -= value % step;
